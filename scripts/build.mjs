...
<head>
  ...
  <style>
    :root { --ink:#13201d; --muted:#66736f; --paper:#f4f1e9; --card:#fffdf8; --line:#d9d9ce; --green:#173f35; --acid:#d8ff64; --shadow:0 18px 50px rgba(26,47,40,.09); }
    * { box-sizing:border-box; }
    ...
    .controls { position:sticky; top:12px; z-index:10; display:grid; grid-template-columns:1fr; gap:12px; padding:12px; background:rgba(255,253,248,.86); border:1px solid rgba(217,217,206,.9); border-radius:18px; box-shadow:var(--shadow); backdrop-filter:blur(18px); }
    ...
    @media (max-width:820px) { .hero-inner,.main { width:min(100% - 24px,1180px); } .hero-copy { padding:50px 0 58px; } .controls { top:6px; } .word-card { grid-template-columns:84px 1fr; grid-template-areas:"rank word" ". definition" "example example"; align-items:start; padding:14px; gap:8px 10px; } .rank-cell { padding-top:8px; gap:12px; } .example { margin-top:3px; padding:9px 0 0; border-left:0; border-top:1px solid #e0e1d7; } .ipa-help { display:none; } footer .main { flex-direction:column; } }
